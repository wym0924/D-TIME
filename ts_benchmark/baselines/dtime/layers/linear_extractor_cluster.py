...
class Linear_extractor_cluster(nn.Module):
    ...
    def _prob_in_top_k(
        ...
        """
        # 确保噪声标准差的最小值，防止除零
        noise_stddev = torch.clamp(noise_stddev, min=1e-6)  # 增加最小值约束
        ...
        # 计算差值并限制范围，避免数值溢出
        diff_in = (clean_values - threshold_if_in) / noise_stddev
        diff_out = (clean_values - threshold_if_out) / noise_stddev
        # 限制在合理范围内 (-100, 100) 避免极端值
        diff_in = torch.clamp(diff_in, min=-100.0, max=100.0)
        diff_out = torch.clamp(diff_out, min=-100.0, max=100.0)
        # is each value currently in the top k.
        normal = Normal(self.mean, self.std)
        prob_if_in = normal.cdf(diff_in)
        prob_if_out = normal.cdf(diff_out)
    ...
    def noisy_top_k_gating(self, x, train, noise_epsilon=1e-2):
        ...
        if torch.isnan(x).any():
            assert "WARNING: Input contains NaN! Replacing with zeros."
        ...
        if torch.isnan(clean_logits).any():
            assert "WARNING: Clean logits contain NaN! Replacing with zeros."
